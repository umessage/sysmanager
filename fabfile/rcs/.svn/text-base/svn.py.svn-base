...
from fabric.api import abort, local, lcd, cd
...
class SVN(object):
    ...
    def export(self, target, user, passwd):
        if user != None and passwd != None:
            local("svn --username=%s --password=%s --non-interactive --trust-server-cert export '%s' '%s'" % (user, passwd, self.path, target))
        else:
            local("svn --non-interactive --trust-server-cert export '%s' '%s'" % (self.path, target))

    def query_revision(self, user, passwd):
        if user != None and passwd != None:
            return local("svn --username=%s --password=%s --non-interactive --trust-server-cert info %s | head -n8 | tail -n1 |cut -d: -f2 | xargs" % (user, passwd, self.path), capture=True).stdout.strip()
        else:
            return local("svn --non-interactive --trust-server-cert info %s | head -n8 | tail -n1 |cut -d: -f2 | xargs" % self.path, capture=True).stdout.strip()
    ...
    def rev(self):
        if not hasattr(self, '_rev'):
            if myenv.cvs_user and myenv.cvs_pass:
                self._rev = self.query_revision(user=myenv.cvs_user, passwd=myenv.cvs_pass)
            else:
    ...
    def iexport(self, target, ver1, ver2, rev1, rev2, user, passwd):
        ...
        if user != None and passwd != None:
            MAfiles, Dfiles = self.MAD(self.diff(old, new, rev1, rev2, user, passwd))
        else:
        ...
        with lcd(target):
            for name in MAfiles:
                ...
                with lcd(name.dirname):
                    if user != None and passwd != None:
                         local('svn --username=%s --password=%s --non-interactive --trust-server-cert export --force %s' % (user, passwd, name.spawn(new.tag)))
                    else:
                         local('svn --non-interactive --trust-server-cert export --force %s' % name.spawn(new.tag))
        return Dfiles
