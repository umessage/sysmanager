...
class Deploy(Task):
    ...
    def post(self):
        ...
        if os.path.exists(env.key_filename):
    ...
    def make_workcopy(self, rc, schema):
        ...
        if myenv.cvs_user and myenv.cvs_pass:
            rc.export(workcopy, user=myenv.cvs_user, passwd=myenv.cvs_pass)
        else:
            rc.export(workcopy)
...
class IncrementalDeploy(Deploy):
    ...
    def deploy(self, ver, rev1=None, rev2=None, *args, **kw):
        ...
        if myenv.cvs_user and myenv.cvs_pass:
            workcopy, Dfiles = self.make_workcopy(rc, schema, ver1, ver, rev1, rev2, user=myenv.cvs_user, passwd=myenv.cvs_pass)
    ...
    @runs_once
    def make_workcopy(self, rc, schema, ver1, ver2, rev1, rev2, user, passwd):
        workcopy = tempfile.mktemp()
        unlink(workcopy)

        Dfiles = rc.iexport(workcopy, ver1, ver2, rev1, rev2, user=user, passwd=passwd)
        schema.mark(rc, workcopy)
